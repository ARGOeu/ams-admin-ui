...
class SchemaUpdate extends React.Component {
  ...
  apiEditSchema(projectName, schemaName, schema) {
    let comp = this;
    this.DM.projectEditSchema(projectName, getShortName(this.state.schema.name), schema).then(r => {
      if (r.done) {
        NotificationManager.info("Schema Updated", null, 1000);
        this.apiGetSchemas(projectName);
        setTimeout(function() {
          comp.props.history.push("/projects/"+projectName+"/schemas/"+schemaName);
        }, 1000);
      } else {
  ...
  initForm() {
    ...
    if (this.state.schema) {
      for (const [key, value] of Object.entries(this.state.schema.schema.properties)) {
        if (this.state.schema.schema.required.includes(key)) {
          attrs.push({"name":key, "value": value["type"], "name_required": true});
        }
        else {
          attrs.push({"name":key, "value": value["type"], "name_required": false});
        }
      }
    }
    p["attributes"] = attrs;
              ...
              <div className="form-group p-3">
                  <FieldArray
                      ...
                      <Button
                          ...
                          onClick={()=> this.handleEditSchema(projectName, getShortName(schemaName), JSON.stringify(msgBody, null, 4))}
                      >
                        Update
                        </Button>
                      {/* <Button
      ...
      <div>
        ...
        <Row>
          ...
          <Col className="text-right">
            ...
            >
              <FontAwesomeIcon icon="times" /> Delete Schema
            </a>
          </Col>
        </Row>
