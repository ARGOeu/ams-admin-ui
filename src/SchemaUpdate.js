      ...
      if (r.done) {
        r.data.schemas.forEach((item, i) => {
          if (getShortName(item.name) === this.props.match.params.schemaname) {
            this.setState({
              schema: item,
              schemaProperties: JSON.stringify(item.schema, null, 4),
              schemaType: item["type"].toUpperCase(),
  ...
  formRequestBody() {
    if (this.state.schemaType === "JSON") {
      if (this.state.schemaProperties) {
        ...
        body["name"] = this.state.schemaName;
        try {
                        ...
                        <CardBody>
                          ...
                          <Row>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                this.setState({
                                  "schemaProperties": JSON.stringify(this.state.schema.schema, null, 4)
                                });
                              }}
                            >
                              Reset
