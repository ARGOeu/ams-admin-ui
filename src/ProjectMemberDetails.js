...
library.add(faExclamationTriangle, faUser, faCrown, faHeartbeat,  faCloudDownloadAlt,
  faCloudUploadAlt,faShieldAlt, faBan);
...
class ProjectMemberDetails extends React.Component {
  constructor(props) {
    super(props);
  ...
  apiDelete(project, username) {
    let comp = this;
    this.DM.projectMemberRemove(project, username).then(done => {
      if (done) {
        NotificationManager.info("Member access revoked", null, 1000);
        setTimeout(function() {
          comp.props.history.push("/projects/details/" + project)
          console.log(comp.props);
        }, 1000);
      } else {
        NotificationManager.error("Error revoking access to member", null, 1000);
      }
    })
  }
      ...
      willDelete = (
        <Card className="border-danger">
          <CardHeader className="border-danger text-danger text-center">
            ...
            <strong>Revoke Access</strong></h5>
          </CardHeader>
          <CardBody  className="border-danger text-center">
            Are you sure you want to revoke access of user
            <strong> {this.state.user.name}</strong> from project 
            <strong> {this.props.match.params.projectname}</strong>
          </CardBody>
        ...
        <Row>
          ...
          <Col className="text-right">
            <Link
              ...
              to={"/projects/" + this.props.match.params.projectname + "/members/update/" + this.state.user.name}
            >
              <FontAwesomeIcon icon="pen" /> Modify Member
            </Link>
            <a
              className="btn btn-danger  ml-1 mr-1"
              href={"/projects/" + this.props.match.params.projectname + "/members/remove/" + this.state.user.name}
            >
              <FontAwesomeIcon icon="ban" /> Revoke User Access
            </a>
          </Col>
        </Row>
